...
unsigned char* toGrayScale(unsigned char* img, int pixelAmount, int channels) {
    unsigned char* imageGrayScale = new unsigned char[pixelAmount]; // 1 channel -> grayscale
...
// function to enhance the readability based on the new calculated grayscale threshold and the grayscale value of a pixel to check if it should be black or white
unsigned char* toEnhanced(unsigned char* image_greyscale, int pixelAmount, int channels, float* threshold) {
    unsigned char* enhanced_img = new unsigned char[pixelAmount];
...
// function to calculte the deviation of each pixel (how much it differs from the mean)
float* calculateDeviation(const unsigned char* img, int pixelAmount, float mean) {
    float* deviation = new float[pixelAmount];
...
float checkMinDeviation(const unsigned char* img, int pixelAmount, float mean, float* deviation) {
    float min_deviation = 255; // Maximum possible deviation
...
float checkMaxDeviation(const unsigned char* img, int pixelAmount, float mean, float* deviation) {
    float max_deviation = 0;   // Minimum possible deviation
...
int* calculateWindowBorder(int width, int height, int windowSize) {
    int* windowBorder = new int[width*height*5];
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            // calculate and store the starting positions, end positions and total pixels covered
            ...
            windowBorder[cur_pixel + 4] = (windowBorder[cur_pixel + 3] - windowBorder[cur_pixel + 1] + 1) * (windowBorder[cur_pixel + 2] - windowBorder[cur_pixel] + 1);
        }
    }
...
// function to calculate the average grayscale value within the window of pixels arround the target pixel
float* calculateWindowMean(const unsigned char* img, int width, int height, int windowSize, int* windowBorder) {
    float* windowMean = new float[width * height];
    int halfWindowSize = windowSize / 2;
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            int cur_pixel = (i * width + j) * 5;
            // get the starting and end points of the window
...
float* calculateWindowStandardDeviation(const unsigned char* img, int width, int height, int windowSize, float* windowMean, float* deviation, int* windowBorder) {
    float* windowStandardDev = new float[width * height];
    int halfWindowSize = windowSize / 2;
...
float* calculateAdaptiveDeviation (float* windowStandardDev, int pixelAmount, float minDev, float maxDev) {
    float* adaptiveDev = new float[pixelAmount];
...
// function to calculate the grayscale threshold for wether a pixel should be fully black or white
float* calculateThreshold(float mean, int pixelAmount, float* meanWindow, float* adaptiveDeviation, float* windowStandardDeviation, float noiseMultiplier) {
    float* threshold = new float[pixelAmount];

    #pragma omp parallel for
    for (int i = 0; i < pixelAmount; ++i) {
        threshold[i] = (meanWindow[i] - (meanWindow[i] * meanWindow[i] - windowStandardDeviation[i]) / ((mean + windowStandardDeviation[i]) * (adaptiveDeviation[i] + windowStandardDeviation[i]))) * noiseMultiplier;
    }
    return threshold;
}
